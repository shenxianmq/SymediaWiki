...
Telexfer 是一款强大的 Telegram 消息管理和网盘转存工具，支持创建多个转存 Bot，实现消息转发、网盘链接转存等功能。(本工具定价 129，直接在捐赠页面捐赠即可)
...
![](https://images.symedia.top/2026/01/09/20260109082543_b1c35e55.png)

1. 进入 **实体列表** 页面
2. 在搜索框中输入频道关键词
3. 点击搜索，找到目标频道或群组
4. 从搜索结果中选择需要操作的实体（系统会显示数字 ID）
5. 选中后保存
...
1. **第一步**：在搜索框中填写频道或群组的**关键词**（如频道名称）
2. **第二步**：点击**搜索**按钮
3. **第三步**：从搜索结果中选择实体，系统会自动填入**数字 ID**
4. **第四步**：确认选择后保存

**❌ 严禁以下操作：**

- ❌ **不要**手动填写 `@` 开头的用户名（如 `@channel_name`）
- ❌ **不要**手动填写 `http://` 或 `https://` 开头的链接
- ❌ **不要**直接复制粘贴 Telegram 链接或用户名

**✅ 正确方式：**

系统搜索后会返回**纯数字 ID**（如：`-1001234567890`），这是唯一正确的实体标识符。只有通过搜索功能获取的数字 ID 才能被系统正确识别和使用。

![](https://images.symedia.top/2025/12/31/20251231222947_1a3f0f92.png)
:::
